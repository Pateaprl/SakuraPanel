      ...
      if (!请求头 || 请求头 !== 'websocket') {
        switch (url.pathname) {
          ...
          case '/get-proxy-status':
            ...
            console.log(`代理启用: ${代理启用}, 代理类型: ${代理类型}, 反代地址: ${反代地址}, SOCKS5账号: ${SOCKS5账号}`);
            ...
            if (代理启用) {
              if (代理类型 === 'reverse') {
                ...
                if (SOCKS5账号) {
                  ...
                  console.log(`SOCKS5测试结果: ${available}`);
                } else {
                  available = false;
                  connectedTo = '未配置';
                }
...
async function 测试代理(连接函数, 描述, env) {
  ...
  try {
    const 测试连接 = await Promise.race([
      连接函数(测试地址, 测试端口),
      new Promise((_, reject) => setTimeout(() => reject(new Error("连接超时")), 5000)) // 5秒超时
    ]);
    await 测试连接.opened;
    console.log(`${描述} 测试成功`);
...
async function 创建SOCKS5(地址类型, 地址, 端口, env) {
  ...
  if (res[1] === 0x02) {
    if (!username || !password) {
      ...
      reader.releaseLock();
      SOCKS5接口.close();
      throw new Error('SOCKS5缺少用户名或密码');
    }
